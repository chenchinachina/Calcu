...
func ArabicToRoman(number int) string {
 if number < 1 {
  panic(errors.New("неверный расчет: результат меньше 1"))
 }
...
func CheckString(input1, input2 string) (interface{}, error) {
 ...
 if arabic1 && arabic2 {
  ...
  panic(errors.New("Некорректные значения. Введите 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X' или цифры от 1 до 10"))
 }
}
...
func Calculate(x int, operator string, y int) int {
 ...
 case "-":
  result = x - y
  if y > x {
   panic(errors.New("Программа не работает с результатами меньше 0"))
  }
 case "*":
  result = x * y
 case "/":
  if y == 0 {
   panic(errors.New("Делить на 0 нельзя!"))
  }
  result = x / y
 default:
  panic(errors.New("Неверный оператор: " + operator))
 }
 return result
}
...
func main() {
 ...
 if err != nil {
  ...
  case []string:
   ...
   resultRoman := ArabicToRoman(calcRoman)
